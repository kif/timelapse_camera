...
class Trajectory(object):
    ...
    def goto_pos(self, pos):
        ...
        self.camera.resume()
        res = OrderedDict([("pan_servo", self.servo_pan.get_config()),
                           ("tilt_servo", self.servo_tilt.get_config())])
        return res
...
class TimeLaps(threading.Thread):
    def __init__(self, resolution=(3280, 2464), framerate=1, delay=20, 
        ...
        if not resolution:
            dico = self.read_config(config_file)
